...
import com.tinkerpop.gremlin.structure.strategy.util.DefaultVertexStrategy;
...
public interface GraphStrategy extends Graph, Graph.Iterators, StrategyWrapped, WrappedGraph<Graph> {
     ...
     */
    @Graph.Helper
    public default DefaultGraphStrategy strategy(final GraphStrategy... strategies) {
    ...
    public default Iterator<Vertex> vertexIterator(final Object... vertexIds) {
        return StreamFactory.stream(getInnerGraph().iterators().vertexIterator(vertexIds)).map(vertex -> this.getStrategy().createVertexStrategy(vertex, this)).iterator();
    }


    public default Iterator<Edge> edgeIterator(final Object... edgeIds) {
        return  StreamFactory.stream(getInnerGraph().iterators().edgeIterator(edgeIds)).map(edge -> this.getStrategy().createEdgeStrategy(edge, this)).iterator();
    }
